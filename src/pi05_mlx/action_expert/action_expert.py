...
class ActionExpert(nn.Module):
    ...
    def __call__(
        ...
        x = mx.concatenate(
            [pg_proj, action_emb], axis=1
        )  # (B, T_prefix + T_action, 1024)
...
class PI05ActionExpert(nn.Module):
    ...
    def _load_expert_weights(
        ...
        """
        src_path = (
            str(Path(repo_id) / "model.safetensors")
    ...
    def flow_matching_sample(
        ...
        """
        B = hidden.shape[0]
        x = mx.random.normal(shape=(B, self.cfg.action_horizon, self.cfg.action_dim))
